...
from impacket.dcerpc.v5.dtypes import (DWORD, GUID, HRESULT, LONG, LPLONG,
                                       LPWSTR, NULL, PGUID, ULONG, ULONGLONG,
                                       USHORT, UUID, WIDESTR, WSTR)
from impacket.dcerpc.v5.ndr import (NDRCALL, NDRPOINTER, NDRSTRUCT,
                                    NDRTLSTRUCT, UNKNOWNDATA,
                                    NDRUniConformantArray)
from impacket.dcerpc.v5.rpcrt import (RPC_C_AUTHN_GSS_NEGOTIATE,
                                      RPC_C_AUTHN_LEVEL_NONE,
                                      RPC_C_AUTHN_LEVEL_PKT_INTEGRITY,
                                      RPC_C_AUTHN_LEVEL_PKT_PRIVACY,
                                      RPC_C_AUTHN_WINNT, DCERPCException,
                                      TypeSerialization1)
...
class INTERFACE:
    ...
    def get_dce_rpc(self):
        return INTERFACE.CONNECTIONS[self.__target][current_thread().name][self.__oxid][
            "dce"
        ]
    ...
    def connect(self, iid=None):
        if (self.__target in INTERFACE.CONNECTIONS) is True:
            if (
                current_thread().name in INTERFACE.CONNECTIONS[self.__target]
                and (
                    self.__oxid
                    in INTERFACE.CONNECTIONS[self.__target][current_thread().name]
                )
                is True
            ):
                dce = INTERFACE.CONNECTIONS[self.__target][current_thread().name][
                    self.__oxid
                ]["dce"]
                ...
                LOG.debug(
                    "Target system is %s and isFQDN is %s"
                    % (self.get_target(), isTargetFQDN)
                )
